---
title: "Stoffdidaktik Mathematik -- Skript zur Vorlesung im Wintersemester 2022/23"
description: "Vorlesungsskript zur Stoffdidaktik Mathematik im Wintersemester 2022/23 an der Universität Potsdam bei Dr. Heiko Etzold"
author: "Dr. Heiko Etzold, Universität Potsdam"
date: "Letzte Änderung: `r format(Sys.time(), '%d.%m.%Y')`"
site: bookdown::bookdown_site
output:
  bookdown::gitbook:
    includes:
      in_header: [replace-references-section-name.html]
    config:
      toc:
        collapse: section 
        scroll_highlight: true
        before: 
          <li><a href="index.html"><b>Stoffdidaktik Mathematik 2022/23</b></a></li>
        after:
          <li><a href="https://creativecommons.org/licenses/by-sa/4.0/" style="display:inline;"><img style="vertical-align:middle;" src="pictures/by-sa.png" height="20px"/></a>Heiko Etzold, 2022</li>
      sharing:
        facebook: false
        twitter: false
        all: false
        github: true
      info: false
    split_bib: true
    split_by: chapter+number
    css: "own.css"
  bookdown::pdf_book:
    keep_tex: true
    toc: true
    includes:
      in_header: "in_header.tex"
      before_body: "before_body.tex"
      after_body: "after_body.tex"
    latex_engine: xelatex
documentclass: scrbook
lang: de-DE
bibliography: "references.bib"
csl: "apa.csl"
link-citations: yes
linkcolor: linkColor
urlcolor: linkColor
github-repo: "heiko-etzold/teaching-stoffdidaktik"
url: "stoffdidaktik.heiko-etzold.de/"
---

# Über dieses Dokument {-}

  
Die Veranstaltung *Stoffdidaktik Mathematik* wird über dieses Dokument begleitet. Es wird fortlaufend aktualisiert und zur Verfügung gestellt. Über ein Git-Respository können Änderungen nachverfolgt werden. In der html-Version gelangt man über die Menüleiste am oberen Rand sowohl zu den Rohdaten als auch zu einer pdf-Version. Die Darstellung der Inhalte ist jedoch optimiert für die html-Version dieses Dokuments.

Zitiere als: 

> Etzold, H. (`r format(Sys.time(), '%Y')`). *Stoffdidaktik Mathematik -- Skript zur Vorlesung im Wintersemester 2022/23* (Version vom `r format(Sys.time(), '%d.%m.%Y')`). https://stoffdidaktik.heiko-etzold.de


Das Vorlesungsskript zur letztjährigen Veranstaltung finden Sie unter https://stoffdidaktik.heiko-etzold.de/2021.


<!-- ## Aufbau des Dokuments {.unnumbered .unlisted} -->

<!-- In den Kapiteln \@ref(stoffdidaktik-mathematik-an-der-up) und \@ref(vier-ebenen-ansatz) werden die begrifflichen und inhaltlichen Grundlagen für den weiteren Verlauf des Skriptes gelegt. Angelehnt an die in Kapitel \@ref(vier-ebenen-ansatz) eingeführte theoretische Grundlage werden in den Kapiteln \@ref(fundamentale-ideen) bis \@ref(zweites-intermezzo-wurzel) einzelne Themen sowohl theoretisch als auch anhand von Praxisbeispielen näher beleuchtet. Die Kapitel \@ref(arbeitsmittel-analysieren) und \@ref(aufgaben-gestalten) legen noch einmal den Fokus auf die Gestaltung von Unterricht unter stoffdidaktischer Perspektive. Im Anhang \@ref(leitidee-zahl) bis \@ref(leitidee-daten-und-zufall) werden anhand der Leitideen bedeutsame Lerngegenstände aufgezählt und ausgewählte Begriffe und Zusammenhänge diskutiert. Anhang \@ref(seminar-und-hausarbeit) gibt Hinweise zum Seminar bzw. der Hausarbeit zur Veranstaltung. -->


## Lizenz {.unnumbered .unlisted}
Soweit nicht anders gekennzeichnet, ist dieses Dokument unter einem Creative Commons Lizenzvertrag lizenziert: »Namensnennung – Weitergabe unter gleichen Bedingungen 4.0 International«. Dies gilt nicht für Zitate und Werke, die aufgrund einer anderen Erlaubnis genutzt werden.
Eine Beschreibung der Lizenz findet sich unter https://creativecommons.org/licenses/by-sa/4.0/deed.de.

Ausgenommen von der CC-BY-SA-Lizenz sind insbesondere die Abbildungen \@ref(fig:FreudenthalWinkel), \@ref(fig:FreudenthalWinkelSpiegeln), \@ref(fig:GVausbilden), \@ref(fig:Tiergehege), \@ref(fig:FlaecheEinheiten), \@ref(fig:GVVerinnerlichen), \@ref(fig:Etappen), \@ref(fig:ACAT) und \@ref(fig:ProzentErkunden) bis \@ref(fig:Schwierig)<!--, \@ref(fig:InnenwinkelReissen), \@ref(fig:ThalesBeweis), \@ref(fig:DefinitionZufallsexperiment) und \@ref(fig:Modelle)--> -- diese werden im Sinne des Zitaterechts ([§ 51 UrhG](https://www.gesetze-im-internet.de/urhg/__51.html)) verwendet.


```{r echo=FALSE}
formal <- function(x) {
  if (knitr::is_latex_output()) {
    sprintf("\\textcolor{formalColor}{%s}", x)
  } else if (knitr::is_html_output()) {
    sprintf("<span style='color: #00A2FF;'>%s</span>", x)
  } else x
}
semantic <- function(x) {
  if (knitr::is_latex_output()) {
    sprintf("\\textcolor{semanticColor}{%s}", x)
  } else if (knitr::is_html_output()) {
    sprintf("<span style='color: #1DB100;'>%s</span>", x)
  } else x
}
concrete <- function(x) {
  if (knitr::is_latex_output()) {
    sprintf("\\textcolor{concreteColor}{%s}", x)
  } else if (knitr::is_html_output()) {
    sprintf("<span style='color: #EE220C;'>%s</span>", x)
  } else x
}
empiric <- function(x) {
  if (knitr::is_latex_output()) {
    sprintf("\\textcolor{empiricColor}{%s}", x)
  } else if (knitr::is_html_output()) {
    sprintf("<span style='color: #F8BA00;'>%s</span>", x)
  } else x
}
```


